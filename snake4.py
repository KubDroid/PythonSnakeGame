...
def reset_game():
    ...
    global snake_position, snake_body, fruit_position, fruit_spawn, direction, change_to, score, game_over 
...
def main():
    ...
    def game_over():
        ...
        # Resto del código de game_over()

        # Registra el puntaje actual en la lista de historial de puntajes
    ...
    while run:
        ...
        # Crear una fuente para mostrar el historial de puntajes
        history_font = pygame.font.SysFont('times new roman', 30)

        # Coordenadas iniciales para mostrar el historial de puntajes
        y_offset = 20
        x_offset = 20

        # Dibuja el historial de puntajes en la pantalla
        for i, score in enumerate(score_history):
            score_text = f'Score {i + 1}: {score}'
            score_surface = history_font.render(score_text, True, white)
            score_rect = score_surface.get_rect()
            score_rect.topleft = (x_offset, y_offset + i * 30)
            game_window.blit(score_surface, score_rect)
